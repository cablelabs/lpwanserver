...
exports.initialize = function (app, server) {
  ...
  /*********************************************************************
     ...
     *      Links records.
     * @apiSuccess {String} object.records.id The Company Network Type Link's Id
     * @apiSuccess {String} object.records.companyId The Company the record is
  ...
  /**
     ...
     * @apiSuccess {Object} object
     * @apiSuccess {String} object.id The Company Network Type Link's Id
     * @apiSuccess {String} object.companyId The Company the record is
     *      linking to the Network Type.
  ...
  app.get('/api/companyNetworkTypeLinks/:id', [restServer.isLoggedIn],
    function (req, res) {
      ...
      modelAPI.companyNetworkTypeLinks.load(id).then(function (np) {
        ...
        .catch(function (err) {
          logger.error('Error getting companyNetworkTypeLink ' + id + ': ', err)
          restServer.respond(res, err)
        })
    })
  ...
  /**
     ...
     * @apiHeader {String} Authorization The Create Session's returned token
     *      prepended with "Bearer "
     * @apiParam (Request Body) {String} companyId The Company the record is
     *      linking to the Network Type.
  ...
  function (req, res) {
    ...
    modelAPI.companyNetworkTypeLinks.load(data.id).then(function (cnl) {
      ...
      .catch(function (err) {
        logger.error('Error getting companyNetworkTypeLink ' + data.id + ': ', err)
        restServer.respond(res, err)
      })
  })
  ...
  function (req, res) {
    ...
    if (req.company.type === 'ADMIN') {
      modelAPI.companyNetworkTypeLinks.remove(id).then(function (ret) {
        ...
        .catch(function (err) {
          logger.error('Error deleting companyNetworkTypeLink ' + id + ': ', err)
          restServer.respond(res, err)
        })
    }
    else {
  ...
  function (req, res) {
    ...
    modelAPI.companyNetworkTypeLinks.pushCompanyNetworkTypeLink(id, req.company.id).then(function (ret) {
      ...
      logger.error('Error pushing companyNetworkTypeLinks ' + id + ': ', err)
      restServer.respond(res, err)
    })
  })
