...
exports.initialize = function (app, server) {
  ...
  function (req, res) {
    ...
    if (req.company.type !== 'ADMIN') {
      ...
      else if (options.companyId !== req.user.company.id) {
    ...
    modelAPI.deviceProfiles.list(options, { includeTotal: true }).then(([ records, totalCount ]) => {
      ...
      .catch(function (err) {
        logger.error('Error getting deviceProfiles: ', err)
  ...
  function (req, res) {
    ...
    if ((req.company.type !== 'ADMIN') &&
      ...
      modelAPI.deviceProfiles.create(
        ...
        .catch(function (err) {
          logger.error('Failed to create deviceProfile ' + JSON.stringify(rec) + ': ', err)
          restServer.respondJson(res, err)
        })
    }
  })
    ...
    modelAPI.deviceProfiles.load(data.id).then(function (dp) {
      ...
      if (changed === 0) {
        ...
        modelAPI.deviceProfiles.update(data).then(function (rec) {
          ...
          .catch(function (err) {
            logger.error('Failed to update deviceProfile ' + data.id + ' with ' + JSON.stringify(data) + ': ', err)
            restServer.respond(res, err)
          })
      }
    })
      .catch(function (err) {
        logger.error('Failed to retrieve deviceProfile ' + data.id + ': ', err)
        restServer.respond(res, err)
      })
  })
  ...
  function (req, res) {
    ...
    modelAPI.deviceProfiles.remove(id, companyId).then(function (ret) {
      ...
      .catch(function (err) {
        logger.error('Error deleting deviceProfile ' + id + ': ', err)
        restServer.respond(res, err)
      })
  })
  ...
  function (req, res) {
    ...
    modelAPI.deviceProfiles.pushDeviceProfile(id, companyId).then(function (ret) {
      ...
      .catch(function (err) {
        logger.error('Error pushing deviceProfile ' + id + ': ', err)
        restServer.respond(res, err)
      })
  })
}
