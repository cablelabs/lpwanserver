...
exports.initialize = function (app, server) {
  ...
  function (req, res) {
    ...
    if ((req.company.type !== 'ADMIN') &&
      ...
      modelAPI.devices.create(rec).then(function (rec) {
        ...
        .catch(function (err) {
          logger.error('Failed to create device ' + JSON.stringify(rec) + ': ', err)
  ...
  function (req, res) {
    ...
    if ((req.company.type === 'ADMIN') ||
      ...
      modelAPI.devices.remove(id).then(function () {
        ...
        .catch(function (err) {
          logger.error('Error deleting device ' + id + ': ', err)
          restServer.respond(res, err)
        })
    }
  ...
  async function unicastDownlinkPostHandler (req, res) {
    ...
    try {
      ...
      logger.error(`Error passing data to device ${id}`, err)
      restServer.respond(res, err)
    }
  }
  app.post(
  ...
  async function ipDeviceUplinkHandler (req, res) {
    ...
    try {
      ...
      logger.error(`Error receiving data from IP device ${devEUI}`, err)
      restServer.respond(res, err)
    }
  }
