...
exports.initialize = function (app, server) {
  ...
  app.get('/api/networkProviders', [restServer.isLoggedIn], function (req, res) {
    ...
    modelAPI.networkProviders.list(options, { includeTotal: true }).then(([ records, totalCount ]) => {
      ...
      .catch(function (err) {
        logger.error('Error getting networkProviders: ', err)
  ...
  app.get('/api/networkProviders/:id', [restServer.isLoggedIn],
    function (req, res) {
      ...
      modelAPI.networkProviders.load(id).then(function (rp) {
        ...
        .catch(function (err) {
          logger.error('Error getting networkProvider ' + req.params.id + ': ', err)
          restServer.respond(res, err)
        })
    })
  ...
  function (req, res) {
    ...
    modelAPI.networkProviders.load(data.id).then(function (rp) {
      ...
      .catch(function (err) {
        logger.error('Error getting networkProvider ' + data.id + ': ', err)
        restServer.respond(res, err)
      })
  })
  ...
  function (req, res) {
    ...
    modelAPI.networkProviders.remove(id).then(function () {
      ...
      .catch(function (err) {
        logger.error('Error deleting networkProvider ' + id + ': ', err)
        restServer.respond(res, err)
      })
  })
}
